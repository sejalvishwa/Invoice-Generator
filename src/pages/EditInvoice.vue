<template>
  ...
  <div class="invoice-container">
    <div class="invoice-card">
      <h1 class="page-title">Edit Invoice #{{ invoice.invoiceNumber }}</h1>
...
export default {
  ...
  methods: {
    ...
    updateInvoice() {
      ...
      const index = invoices.findIndex(inv => inv.id == this.$route.params.id)
      
      if (index !== -1) {
        invoices[index] = { ...this.invoice, subtotal: this.subtotal, tax: this.tax, total: this.total }
  ...
  mounted() {
    const invoices = JSON.parse(localStorage.getItem('invoices') || '[]')
    const foundInvoice = invoices.find(inv => inv.id == this.$route.params.id)
